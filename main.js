...
// Load training list from database

ipcMain.on("LoadTrainingList" , (event)=>{
  let result= knex.select().from("trainings")
  result.then((row)=>{
...
ipcMain.on("addTrainingToDB", (event, args) => {
   knex("trainings")
    .insert({
      name: args[0],
      date: args[1],
    ...
    .then(() => {
      // showMessageBoxSync  
      dialog.showMessageBoxSync({...options , "detail" : "Training added succesfully "} )
      event.sender.send("reload");
    });
});


//get Training info from DB to fill inputs modal

ipcMain.on("getModelTrainingInput", (event, id) => {
  let result = knex.select().from("trainings").where("id", id);
  result.then((row) => {
    event.sender.send("inputModelTrainingLoaded", row);
  });
});

//  update Training from modal

ipcMain.on("updateTraining", (event, args) => {
  knex("trainings").where("id", args[0]).update({
    name:args[1],  
    date:args[2],
    trainer:args[3],
    sub:args[4]
    ...
    dialog.showMessageBoxSync({
      ...options,
      detail: "Training update succesfully",
    }),
    event.sender.send("reload")
  )
});

// message box option training

const optionsTraining = {
  type: "info",
  message: "Message",
  button: "OK",
  detail: "Training has been delete",
};

// delete training from database with id

ipcMain.on("deleteTraining", (event, id) => {
  knex("trainings")
    .where("id", id)
    .del()
    .then(() => {
      dialog.showMessageBoxSync(optionsTraining);
      event.sender.send("reload");
    });
});
